...
04-Working/          ← L1 当前工作
├── YYYY-MM-DD/
│   └── refine-*.md          ← 会话摘要（processed: false = 未处理）
03-Episodic/         ← L2 情景记忆
...
激活时读取 `~/.claude/second-brain/.vault-config.json`，获取 `vaultPath`、`initialized`、`hooksMounted` 三个字段。
...
1. **Vault 路径确认** — 从 `.vault-config.json` 读取 `vaultPath`（如果存在）。首次使用无配置时，询问用户 vault 路径（默认：`~/Documents/SecondBrain`），用户确认后执行：
   ```bash
   second-brain-cli init-vault <vault-path>
   ```
   脚本创建目录结构、生成模板、更新 `~/.claude/second-brain/.vault-config.json` 中 `initialized: true` 和 `vaultPath`

2. **自动挂载 Hooks** — 检查 `.vault-config.json` 中 `hooksMounted` 是否为 `true`。未挂载时自动执行：
   ```bash
   second-brain-cli mount-hooks
   ```
   脚本更新 `~/.claude/settings.json`，设置 `~/.claude/second-brain/.vault-config.json` 中 `hooksMounted: true`

   > **无需用户确认**：这是 Skill 正常运行所必需的基础配置，`npx skills add` 不会自动注册 hooks。

3. 向用户报告初始化完成（Vault 路径、Hooks 状态）
...
- **索引格式**: 各索引的列定义见 `references/vault-templates/` 中对应模板的 HTML 注释
- **Identity 模板**: `references/vault-templates/`
- **会话摘要格式**: `references/session-summary-template.md`
- **Identity 提案**: 提案格式、状态机、去重规则见模板 HTML 注释

...
1. **`.vault-config.json`** — 检查 `initialized` 是否为 `true`
2. **`~/.claude/settings.json`** — 检查 `hooks.Stop` 和 `hooks.SessionStart` 是否已注册
...
2. **自动挂载 Hooks** — 如果 `settings.json` 中未检测到已注册的 hooks，自动执行：
   ...
   ```
   脚本更新 `~/.claude/settings.json`，设置 `.vault-config.json` 中 `hooksMounted: true`

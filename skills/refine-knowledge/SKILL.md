...
1. **`.vault-config.json`** — 位于 skills 根目录，检查 `initialized` 是否为 `true`
2. **`~/.claude/settings.json`** — 检查 `hooks.Stop` 和 `hooks.SessionStart` 中是否已包含 `queue-session` 和 `inject-context`
...
1. **Vault 路径确认** — 询问用户 vault 路径（默认：`~/Documents/obsidian-workspace/obsidian_workspace`），用户确认后执行：
   ```bash
   npx tsx ../scripts/init-vault.ts <vault-path>
   ```
   脚本创建目录结构、生成模板、更新 `.vault-config.json` 中 `initialized: true` 和 `vaultPath`

2. **自动挂载 Hooks** — 如果 `settings.json` 中未检测到已注册的 hooks，自动执行：
   ```bash
   npx tsx ../scripts/mount-hooks.ts
   ```

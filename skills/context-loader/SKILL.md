...
2. **`~/.claude/settings.json`** — 检查 `hooks.SessionStart` 是否已包含 `inject-context`

- 均已配置：正常执行上下文加载
- 任一未配置：提示用户"Second Brain 尚未完成初始化，是否现在执行？"
  - 用户确认：执行 refine-knowledge 的自动初始化流程（`npx tsx ../scripts/init-vault.ts` + `npx tsx ../scripts/mount-hooks.ts`）

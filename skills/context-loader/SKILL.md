...
hooks:
  SessionStart:
    - matcher: ""
      hooks:
        - type: command
          command: "npx --yes second-brain-cli-session-start-hook"
---
...
2. **`~/.claude/settings.json`** — 检查 `hooks.SessionStart` 是否已注册（包含 `second-brain-cli-session-start-hook`）

- 均已配置：正常执行上下文加载
- 任一未配置：提示用户"Second Brain 尚未完成初始化，是否现在执行？"
  - 用户确认：执行 refine-knowledge 的自动初始化流程（`second-brain-cli init-vault` + `second-brain-cli mount-hooks`）
